...
AnimationPlayer::AnimationPlayer(BaseApp& baseApp, Model& model, bool interpolationEnabled) : BaseComponent(baseApp),
								 mModel(&model), mCurrentClip(nullptr), mCurrentTime(0.0f), mCurrentKeyframe(0U), mToRootTransforms(), mFinalTransforms(),
								 mInverseRootTransform(MatrixFuncs::Identity), mInterpolationEnabled(interpolationEnabled), mIsPlayingClip(false), mIsClipLooped(true)
{
...
void AnimationPlayer::Update(const Time& deltaTime)
{
	if (mIsPlayingClip)
	{
		assert(mCurrentClip != nullptr);

		mCurrentTime += static_cast<float>(deltaTime.ElapsedTime()) * mCurrentClip->TicksPerSecond();
