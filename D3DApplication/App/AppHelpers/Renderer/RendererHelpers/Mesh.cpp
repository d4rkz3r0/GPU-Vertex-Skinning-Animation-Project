...
Mesh::Mesh(Model& model, aiMesh& mesh) :
	...
	mVertices(), mNormals(), mTangents(), mBiNormals(), mTextureCoordinates(), mVertexColors(),
	mFaceCount(0), mIndices(), mBoneWeights(), mVertexBuffer(), mIndexBuffer()
{
	...
	if (mesh.HasBones())
	{
		...
		for (UINT i = 0; i < mesh.mNumBones; i++)
		{
			...
			UINT boneIndex = 0U;
			string boneName = meshBone->mName.C_Str();
...
Mesh::~Mesh()
{
	for (vector<XMFLOAT3>* textureCoordinates : mTextureCoordinates)
	{
		delete textureCoordinates;
	}

	for (vector<XMFLOAT4>* vertexColors : mVertexColors)
	{
		delete vertexColors;
	}
...
const vector<UINT>& Mesh::Indices() const
{
	return mIndices;
}
...
BufferWrapper& Mesh::VertexBuffer()
{
	return mVertexBuffer;
}

BufferWrapper& Mesh::IndexBuffer()
{
	return mIndexBuffer;
}
...
bool Mesh::HasCachedVertexBuffer() const
{
	...
	return mesh->mVertexBuffer.GetBuffer() != nullptr;
}
...
bool Mesh::HasCachedIndexBuffer() const
{
	...
	return mesh->mIndexBuffer.GetBuffer() != nullptr;
}
...
void Mesh::CreateIndexBuffer(ID3D11Buffer** indexBuffer)
{
	...
	mModel.GetBaseApp().D3DDevice()->CreateBuffer(&indexBufferDesc, &indexSubResourceData, indexBuffer);
}
