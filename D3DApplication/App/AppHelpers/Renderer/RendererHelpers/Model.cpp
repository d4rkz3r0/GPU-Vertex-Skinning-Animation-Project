...
Model::Model(BaseApp& baseApp, const string& filename, bool flipUVs) : mBaseApp(baseApp), mMeshes(), mMaterials(), mAnimations(), mBones(), mBoneIndexMapping(), mRootNode(nullptr)
{
	...
	if (scene->HasAnimations())
	{
		...
		for (UINT i = 0; i < scene->mNumAnimations; i++)
		{
			...
			mAnimationsByName.insert(pair<string, AnimationClip*>(animationClip->Name(), animationClip));
		}
	}
...
const vector<AnimationClip*>& Model::Animations() const
{
	return mAnimations;
}

const map<string, AnimationClip*>& Model::AnimationsbyName() const
{
	return mAnimationsByName;
}

const vector<Bone*> Model::Bones() const
{
	return mBones;
}

const map<string, UINT> Model::BoneIndexMapping() const
{
